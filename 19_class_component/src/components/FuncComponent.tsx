...
const Counter1: React.FC = () => {
    ...
    //React.useEffect()
    useEffect(() => {
        console.log('komponent created'); // analog componentDidMounth() s classComponent, schto budet petschtatsa pri sosdanii stranizi
        
        return() => {
            console.log('component deleted'); // analog componentWillUnmounth() w classComponent, schto budet petschatatsa pri udalenii dannich
        }
    }, []);

    //React.useEffect() 2 variant
    useEffect(() => {
        console.log('component updated'); //analog componentDidMounth() w classComponent
    }, [count]);//propisatj w [] satschem imenno nuschno sledit, on sledit teper tolko sa dejstwijami countera
